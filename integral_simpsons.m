function y = integral_simpsons(x, f_x)
    
% % Use extended Simpson's rule for the numerical integration
%     N = length(x);
%     y = 3/8*(f_x(1)+f_x(N)) + 7/6*(f_x(2)+f_x(N-1)) + 23/24*(f_x(3)+f_x(N-2));
%     dx = x(2) - x(1);
%     y = dx*(y + sum(f_x(4:(N-3))));
%     %y = (y + sum(f_x(4:(N-3))));
   
%Use MM numerical integration
s = mmintgrl(x, f_x);
y = s(end);
