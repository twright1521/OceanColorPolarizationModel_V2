...
a_pico_lam = interp1(wav_pico,a_pico_file,443,'linear','extrap');
a_micro_lam = interp1(wav_pico,a_micro_file,443,'linear','extrap');

Sf = (a_star_phy_lam - a_micro_lam)/(a_pico_lam - a_micro_lam);
a_star_phy = Sf.*a_pico + (1-Sf).*a_micro;
a_phy = a_star_phy.*CHL;

a_phy_443 = interp1(wav,a_phy,443,'linear','extrap');
