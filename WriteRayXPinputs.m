...
%||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
Sim_num = 1;        % Denotes the simulation folder number
...
for zeta = 1:length(CHL)
    ...
    [a_phy,a_CDOM,a_NAP,b_phy,bb_phy,b_NAP,bb_NAP,a_water,bb_water,b_water] = getAbsorptionBackscatteringVersion3(CHL(zeta),wav,1.5);
