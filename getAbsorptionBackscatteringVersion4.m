function [a_phy,a_CDOM,a_NAP,b_phy,bb_phy,b_NAP,bb_NAP,a_water,bb_water,b_water] = getAbsorptionBackscatteringVersion4(CHL,wav,zeta)
...
a_pico_lam = interp1(wav_pico,a_pico_file,443,'spline','extrap');
a_micro_lam = interp1(wav_pico,a_micro_file,443,'spline','extrap');

Sf = (a_star_phy_lam - a_micro_lam)/(a_pico_lam - a_micro_lam);
a_star_phy = Sf.*a_pico + (1-Sf).*a_micro;
a_phy = a_star_phy.*CHL;

a_phy_443 = interp1(wav,a_phy,443,'spline','extrap');
...
% Small Particles
Ps = 0.0244*CHL^0.8497;
b_ps = 0.039.*Ps.*(1.1513).*(400./wav).^1.7;

% Large Particles
Pl = 0.4639*CHL^0.7025;
b_pl = 0.00064.*Pl.*(0.3411).*(400./wav).^0.3;
